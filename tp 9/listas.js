...
function invertirLista(listaDeNumeros) {
    listareversa = listaDeNumeros.reverse()
    return listareversa
}
...
function calcularPromedio(listaDeNumeros) {
    ...
    promedio = total / listaDeNumeros.length

    return promedio
}
console.log("calcularPromedio([2,3,4]): ", calcularPromedio([2, 3, 4]))
console.log("calcularPromedio(8,5,19,9): ", calcularPromedio([8, 5, 19, 9]))
...
function triplicarLista(listaDeNumeros) {
    total = 0
    for (let i = 0; i < listaDeNumeros.length; i++) {
        total = listaDeNumeros[i] * 3
        listaDeNumeros[i] = total
    }
...
function ordenarDeMayorAMenor(listaDeNumeros) {
    let listaordenada = listaDeNumeros.sort()
    let reversa = listaordenada.reverse()
    return reversa
}
...
function ordenarPalabrasPorLongitud(listaDePalabras) {
    let reversa = listaDePalabras.sort();
    return reversa
}
...
function encontrarPalabraMasCorta(listaDePalabras) {
    ...
    for (let i = 0; i < listaDePalabras.length; i++) {
        if (listaDePalabras[i].length < palabramin.length) {
...
function filtrarSoloPositivos(listaDeNumeros) {
    ...
    for (let i = 0; i < listaDeNumeros.length; i++) {
        if (listaDeNumeros[i] > 0 ){
            palabrapos = listaDeNumeros[i]
        }
    }
    return palabrapos
}
console.log("filtrarSoloPositivos([1,-1,0]): ", filtrarSoloPositivos([1, -1, 0]))
console.log("filtrarSoloPositivos(listaNumerosEjemplo): ", filtrarSoloPositivos(2,3,4,0,-24,-1,2))
